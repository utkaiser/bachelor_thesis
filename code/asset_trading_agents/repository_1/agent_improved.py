...
class Abstract_Agent(ABC):
    ...
    def __init__(self, trading_env):
        ...
        self.last_improvement_index = 15 #if index>15, then break ???????????????????????
    ...
    def take_action_training(self, t, df_training):
        # buy
        if self.action> 0 and t < (len(df_training) - 15):
            if self.current_cash >= self.action*df_training[t][0]* (1 + self.transaction_cost):
                for i in range(self.action):
                    self.inventory.append(df_training[t][0])
                self.current_cash -= df_training[t][0]* self.action * (1 + self.transaction_cost)
            else:
                #if not enough money to buy
                max_buy = int(self.current_cash // df_training[t][0])
                for i in range(max_buy):
                    self.inventory.append(df_training[t][0])
                self.current_cash -= df_training[t][0]* max_buy * (1 + self.transaction_cost)

        # sell
        elif self.action< 0:
            if len(self.inventory)>=self.action:
                for i in range(self.action):
                    del self.inventory[0]
                self.current_cash += df_training[t][0]* self.action * (1-self.transaction_cost)
            else:
                #if not enough assets in inventory
                max_sell = len(self.inventory)
                for i in range(max_sell):
                    del self.inventory[0]
                self.current_cash += df_training[t][0] * max_sell * (1 - self.transaction_cost)

        self.starting_money = len(self.inventory) * df_training[t][0] + self.current_cash
    ...
    def take_action_testing(self,t,df_test):
        # buy
        if self.action > 0 and t < (len(df_test) - 15):
            if self.current_cash >= self.action * df_test[t][0]* (1 + self.transaction_cost):
                for i in range(self.action):
                    self.inventory.append(df_test[t][0])
                self.current_cash -= df_test[t][0] * self.action * (1 + self.transaction_cost)
            else:
                # if not enough money to buy
                max_buy = int(self.current_cash // df_test[t][0])
                for i in range(max_buy):
                    self.inventory.append(df_test[t][0])
                self.current_cash -= df_test[t][0] * max_buy * (1 + self.transaction_cost)
            self.transaction_sum += df_test[t][0] * self.transaction_cost
        ...
        # sell
            ...
            if len(self.inventory) >= self.action:
                ...
                for i in range(max_sell):
                    del self.inventory[0]
                self.current_cash += df_test[t][0] * max_sell * (1 - self.transaction_cost)
            self.transaction_sum += df_test[t][0] * self.transaction_cost
            self.states_sell.append(t)

        self.starting_money = len(self.inventory) * df_test[t][0] + self.current_cash
        self.portfolio_movement.append(self.current_cash + len(self.inventory) * df_test[t][0])
